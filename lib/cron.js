(function (root, factory) {
	...
	CronTime.prototype = {
		 ...
		 * get next date that matches parsed cron time
		 */
		_getPreviousDateFrom: function (start) {
			var date = moment(start);
			if (date.toString() == 'Invalid date') {
				console.log('ERROR: You specified an invalid date.');
				return date;
			...
			while (true) {
				...
				if (!(date.hours() in this.hour)) {
					origDate = moment(date);
					date.hours(date.hours() == 0 && diff > 86400000 ? 23 : date.hours() - 1);
					date.minutes(0);
					date.seconds(0);
					continue;
				}
				...
				if (!(date.minutes() in this.minute)) {
					origDate = moment(date);
					date.minutes(date.minutes() == 0 && diff > 60 * 60 * 1000 ? 59 : date.minutes() - 1);
					date.seconds(0);
					continue;
				}
				...
				if (!(date.seconds() in this.second)) {
					origDate = moment(date);
					date.seconds(date.seconds() == 0 && diff > 60 * 1000 ? 59 : date.seconds() - 1);
					continue;
				}
		 ...
		 * get next date that is a valid DST date
		 */
		_findPrevDST: function (date) {
			var newDate = moment(date);
			while (newDate >= date)
				newDate.subtract(1, 's');
